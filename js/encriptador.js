function encriptar(traduccion){
    document.querySelector("#atento").removeAttribute("style");
    ...
    if (texto != ""){
        ...
        for(var i=0; i < texto.length; i++){
            if(((texto[i] < 'a') || (texto[i] > 'z')) && (texto[i] != ' ')){
                document.querySelector("#atento").style.color = "red";/*funcion advertencia mayusculas*/
                document.querySelector("#atento").style.fontSize = "20px";
                return;
            }
...
function desencriptar(traduccion){
    ...
    if (texto != ""){
        for(var i=0; i < texto.length; i++){
            if(((texto[i] < 'a') || (texto[i] > 'z')) && (texto[i] != ' ')){
                document.querySelector("#atento").style.color = "red";
                document.querySelector("#atento").style.fontSize = "20px";
                return;
            }
            else if((texto.length == 1 && texto == " ") || texto.replace(/ /g, "") == ""){
